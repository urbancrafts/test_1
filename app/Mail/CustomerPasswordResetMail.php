...
class CustomerPasswordResetMail extends Mailable
{
    ...
    public function build()
    {
        ...
        $emailContent = EmailTemplate::where('title', 'Password Reset Success')->get()->first();
        //$verifyCode = UserVerification::insertEmailCode($this->data->id)->email_code;
        //
        $siteInfo = Settings::single();

        $search = array('{NAME}');
        $replace = array($this->data->first_name);
        ...
        ->view('mails.generic', ['receiverName' => $this->data->name,
        'site_url' => $siteInfo->site_url,
        'site_name' => $siteInfo->site_name,
        'site_logo' => $siteInfo->site_logo,
        'title' => $emailContent->title,
