...
function createTransactionRecord(trn_type,ref_id,gateway,name,email,phone,user_id,curr,amount,status,msg){
                    ...
                    'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
                      ...
                      type:"POST",
                      url: "{{ action('App\Http\Controllers\AjaxRequestController@create_subscription_transaction_record') }}",
                      dataType: "json",
                      ...
                      success:function(data){
                          if(data.success == true){
                           window.location = "{{ url('member_dashboard') }}/"+user_id; 
                          }else{
...
function payWithPaystack(){
    var handler = PaystackPop.setup({
      ...
      amount: {{ $settings[0]->membership_fee }} * 100,
         ...
         custom_fields: [
            {
                ...
                value: "+234{{Auth::user()->phone}}"
            }
         ]
      },
      callback: function(response){
          
          createTransactionRecord('Membership',response.reference,'paystack','{{Auth::user()->name}}','{{Auth::user()->email}}','{{Auth::user()->phone}}','{{Auth::user()->id}}','{{$settings[0]->curr}}','{{$settings[0]->membership_fee}}','success','test transaction');
        ...
        <div class="row">
            ...
            <div class="card-payment paystack">
             ...
             <div id="paymentSection">
              ...
              <h4>{{ $settings[0]->curr }}{{ $settings[0]->membership_fee }} / </h4>
              ...
              <h4>{{ $settings[0]->curr }}{{ $settings[0]->membership_fee }} / </h4>
                        <ul>
                            ...
                            <li>
                                ...
                                <input type="hidden" value="{{$settings[0]->curr }}" name="curr" id="curr" />
                                <input type="hidden" value="{{ $settings[0]->membership_fee }}" name="amount" id="amount" />
