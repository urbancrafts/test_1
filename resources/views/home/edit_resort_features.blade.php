     ...
     $.ajax({
      ...
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         beforeSend:function(){
             ...
             $('.btn-primary').html("<img src='{{ asset('loaders/AjaxLoader.gif') }}' />");
         ...
         success:function(data){
            if(data.success == true){
             ...
             $(".btn-primary").prop('disabled', false);
             $('.btn-primary').html("SAVE AND CONTINUE");
             window.location = "{{ url('') }}/admin/resort_manager/create_room/"+data.data.resort_id;
...
<div class="wrapper">   
  ...
  <li class="nav-item">
    <a class="nav-link active" href="{{url('admin/resort_manager/edit_resort_features/'.$resorts2[0]->id)}}">Features</a>
  </li>
  <li class="nav-item">
  ...
  <!-- <li class="nav-item">
   ...
   -->
          ...
          <form id="feature-form" method="post" action="{{ action('App\Http\Controllers\ResortController@update_resort_features') }}">
            <input hidden name="resort" value="{{$resorts2[0]->id}}" />
            <div class="form-group">
              <fieldset class="resort-features">
                ...
                @foreach ($admin_features as $admin_feature)
                                              
                <label><input type="checkbox" name="feature[]" id="feature" class="minimal" value="{{$admin_feature->feature}}" /> {{$admin_feature->feature}}</label> |   
                ...
                <label id="feature_elem_{{$key}}">{{$features}}<a href="#" class="fas fa-trash" onclick="removeFeatures('{{$resorts2[0]->id}}','{{$key}}')" title="Delete {{$features}}"></a></label>        
                @endforeach
                @endif
                @endif
              </fieldset>
