...
@section('content')
        ...
        function loadHomeBlog($record_per_page, $start){
          ...
          $action = "loadHomeBlogs";
            ...
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
               ...
               type: "POST",
               dataType: "json",
               url: "{{ action('App\Http\Controllers\AjaxRequestController@load_profile_blog') }}",
               data: {"record_per_page": $record_per_page, "start": $start, "user_id": $user_id, "action": $action},
               beforeSend:function(){
                 $("#activity").append("<div class='load'><img src='{{ asset('loaders/loader.gif')}}'/ > Loading...</div>");
               },
               complete:function(){
                 $(".load").remove();
               },
               error:function(){
               $("#activity").append("<div class='errLoading'><img src='{{asset('icons/ic_connections.png')}}'/ > Please check your internet connection</div>");
               },
 ...
 <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
          @if (Auth::user()->id == $profiles[0]->id)
              My Profile
          @else
        {{$profiles[0]->name}}'s Profile
          @endif
      </h1>
        ...
        <div class="col-md-3">
          ...
          <div class="card card-primary card-outline">
            <div class="card-body box-profile">
              <div class="text-center">
                @if($profiles[0]->img == null)
                <img class="profile-user-img img-fluid img-circle" src="{{asset('storage/img/users/default.png')}}" alt="{{$profiles[0]->name}}">   
                @else
                <img class="profile-user-img img-fluid img-circle" src="{{asset('storage/img/users/'.$profiles[0]->id.'/profile/'.$profiles[0]->img)}}" alt="{{$profiles[0]->name}}">  
                @endif
              </div>

              <h3 class="profile-username text-center">{{$profiles[0]->name}}</h3>

              <p class="text-muted text-center"></p>
              
              @if($profiles[0]->id == Auth::user()->id || Auth::user()->user_type == "admin")
              <form id="profile-img-form" action="{{ action('App\Http\Controllers\AjaxRequestController@upload_profile_image') }}" method="POST" enctype="multipart/form-data">
              <input type="hidden" id="user_id" name="user_id" value="{{$profiles[0]->id}}" />
          ...
          <div class="card card-primary">
            ...
            <div class="card-body">
              ...
              <p class="text-muted">
                @if ($profiles[0]->user_type == "member")
                   Club Member
                @elseif($profiles[0]->user_type == "resort_owner")
                  Resort Owner
                @elseif($profiles[0]->user_type == "boat_owner")
                  Boat Owner
                @elseif($profiles[0]->user_type == "yacht_owner")
                  Yacht Owner
                @elseif($profiles[0]->user_type == "admin")
                  Admin User
                @endif
              </p>
          ...
          <div class="card">
            ...
            <div class="tab-content">
              ...
              <div class="tab-pane" id="settings">
                 @if(Auth::user()->id == $profiles[0]->id || Auth::user()->role == 1)
                <form id="profile-update-form" action="{{ action('App\Http\Controllers\AjaxRequestController@profile_update_upload') }}" class="form-horizontal" method="POST">
                 
                  <div class="form-group">
                    <input type="hidden" name="user_id" value="{{$profiles[0]->id}}" />
                    <label for="inputName" class="col-sm-2 control-label">Name</label>

                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="p_name" name="p_name" placeholder="Enter your name" value="{{$profiles[0]->name}}" required>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="email" class="form-control" id="p_email" name="p_email" placeholder="Enter your email address" value="{{$profiles[0]->email}}">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="number" class="form-control" id="p_number" name="p_number" placeholder="Enter your phone number" value="{{$profiles[0]->phone}}">
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <textarea class="form-control" id="about" name="about" placeholder="A brief bio data">{{$profiles[0]->about}}</textarea>
                    </div>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="text" class="form-control" id="occupation" name="occupation" value="{{$profiles[0]->profession}}" placeholder="Enter occupation">
                    </div>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <select id="position" name="position" class="form-control">
                        @if ($profiles[0]->position != "")
                      <option value="{{$profiles[0]->position}}" selected>{{$profiles[0]->position}}</option>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      ...
                      <select id="privilege" name="privilege[]" class="form-control select2" multiple="multiple" data-placeholder="Select addition privileges">
                        ...
                        @if($profiles[0]->privilege == "resort_owner" && $profiles[0]->privilege_2 == "")
                        <option value="resort_owner" selected>Resort Owner</option>
                        <option value="boat_owner">Boat Owner</option> 
                        @elseif($profiles[0]->privilege_2 == "boat_owner" && $profiles[0]->privilege == "")
                        <option value="boat_owner" selected>Boat Owner</option>
                        <option value="resort_owner">Resort Owner</option> 
                        @elseif($profiles[0]->privilege == "resort_owner" && $profiles[0]->privilege_2 == "boat_owner")
                        <option value="resort_owner" selected>Resort Owner</option>
                        <option value="boat_owner" selected>Boat Owner</option> 
                        @else
                        <option value="boat_owner">Boat Owner</option>
                        ...
                        @endif
                       @elseif($profiles[0]->user_type == "resort_owner")
                       @if($profiles[0]->privilege_2 == "boat_owner")
                       <option value="boat_owner" selected>Boat Owner</option>  
                       @else
                       <option value="boat_owner">Boat Owner</option> 
                       @endif
                       ...
                       @elseif($profiles[0]->user_type == "boat_owner")
                       @if($profiles[0]->privilege == "resort_owner")
                       <option value="resort_owner" selected>Resort Owner</option>
                       @else
                       <option value="resort_owner">Resort Owner</option> 
                       @endif
                   ...
                   @if($profiles[0]->id != Auth::user()->id )
                       @if($profiles[0]->isActive == 1)
...
<span class="alert alert-success show-success1" style="display: none"></span>
          ...
          @endif
              ...
              <div class="tab-pane" id="security">
                ...
                <form id="profile-password-change" action="{{ action('App\Http\Controllers\AjaxRequestController@profile_change_password') }}" class="form-horizontal" method="POST">
                  <div class="form-group">
