      ...
      <div class="row">
        ...
        <section class="col-lg-7 connectedSortable" id="blog-body">
            ...
            <div class="card card-info">
                ...
                <div class="card-body">
                  <form id="create-debt-form" action="{{ action('App\Http\Controllers\AjaxRequestController@post_member_credit') }}" method="post" enctype="multipart/form-data">
                ...
                <option value="{{$user->id}}">{{$user->name}}</option>   
                   ...
                   <div class="form-group">
                      ...
                      <span>{{$settings[0]->curr}}</span><input type="number" class="form-control" id="amount" name="amount" placeholder="Enter Credit Amount" min="1" >
                    </div>
          ...
          <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tr>
                ...
                <td>{{$debt->curr}}{{$debt->amount}}</td>
                <td>{{$debt->descr}}</td>
                <td>@if ($debt->settled == 1)
                    yes
                  @else
                  no
                  @if ($debt->created_by == Auth::user()->id)
                  <a href="#" onclick="updateCredit('{{$debt->id}}','{{$debt->user_name}}')" class="btn btn-special" title="Update this roll">Update</a>
                  @endif
                @endif
