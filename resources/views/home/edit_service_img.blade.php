...
if(str == true){
  $.ajax({
      ...
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         type: "POST",
         dataType: "json",
         url: "{{ action('App\Http\Controllers\ServiceController@remove_service_img') }}",
...
<div class="wrapper">   
          ...
          <div class="tabSide">
            <div class="card card-body p-0" style="border: 2px solid #E51924; border-radius: 0px">
                <ul class="nav nav-pills nav-justified">
                  ...
                  <li class="nav-item">
                    <a class="nav-link" href="{{url('admin/service_manager/edit_service/'.$services[0]->id)}}">Edit form</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link active" href="{{url('admin/service_manager/edit_service_img/'.$services[0]->id)}}">Images</a>
                  </li>
                  <li class="nav-item">
  ...
  <!-- <li class="nav-item">
   ...
   -->
          ...
          <form id="img-form" method="post" action="{{ action('App\Http\Controllers\ServiceController@update_service_img') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row firstBankDel mt-4" id="first_bank">
                <div class="col-xl-4">
                 ...
                 @if($services[0]->images)
                  @foreach($serviceImages as $key => $pimg)
                      ...
                      <div class="card mb-0">
                          <div class="card-header">
                              <div class="d-flex justify-content-between">
                                  ...
                                  <div>
                                      <button onclick="removeImg('{{$services[0]->id}}', '{{$key}}')" class="btn btn-danger btn-sm" type="button"><i class="fa fa-trash"></i></button>
                                  </div>
                              </div>
            ...
            <a href="{{url('admin/service_manager/bookings/'.$services[0]->id)}}" class="btn btn-primary rounded-0 pt-3 pb-3" type="submit">SAVE AND CONTINUE</a>
            </div>
