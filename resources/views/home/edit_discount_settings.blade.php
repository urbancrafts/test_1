...
if($.trim($('#discount-number').val()) == ""){
     ...
     $.ajax({
      ...
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         beforeSend:function(){
             $("#save-btn").prop('disabled', true);
             $('#save-btn').html("<img src='{{ asset('loaders/AjaxLoader.gif') }}' />");
         ...
         success:function(data){
            if(data.success == true){
             ...
             $("#save-btn").prop('disabled', false);
             $('#save-btn').html("SAVE AND CONTINUE");
             window.location = "{{ url('') }}/admin/shop_manager/edit_delivery_settings/"+shop;
...
<div class="wrapper">   
          ...
          <div class="tabSide">
            <div class="card card-body p-0" style="border: 2px solid #E51924; border-radius: 0px">
                <ul class="nav nav-pills nav-justified">
                  ...
                  <li class="nav-item">
                    <a class="nav-link active" href="{{url('admin/shop_manager/edit_discount_settings/'.$shops[0]->id)}}">Discount</a>
                  </li>
                  <li class="nav-item">
  ...
  <!-- <li class="nav-item">
   ...
   -->
          ...
          <form id="discount-form" method="post" action="{{ action('App\Http\Controllers\ShopController@update_product_discount') }}">
            <input hidden name="shop" id="shop" value="{{$shops[0]->id}}" />
                    ...
                    <div class="col-sm-6">
                      ...
                      <div class="form-group">
                        ...
                        <span style="color: red;" class="shop-discount-number-error">*</span>
                          <input type="number" id="discount-number" name="discount-number" class="form-control" onfocus="elementFocus(this.id, 'shop-discount-number-error')" placeholder="Enter discount number: 10" value="{{$shops[0]->discount_number}}" >
                      </div>
                    </div>
                    <div class="col-sm-6">
                  <div class="form-group">
                    <label>Discount Percent:</label>
                      ...
                      <input type="number" class="form-control" id="discount-percent" name="discount-percent" onfocus="elementFocus(this.id, 'shop-discount-percent-error')"  placeholder="Enter percent: 10(%)" value="{{$shops[0]->discount_percent}}" >
                    </div>
                    </div>
                  </div>
