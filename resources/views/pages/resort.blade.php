@extends('layouts.app1')
      ...
      @section('content')
        ...
        <style id='foxuries-style-inline-css' type='text/css'>
            div.foxuries-resort{
              background-image: url({{ $resorts[0]->img_1 }});
            }
            </style>    
...
<div class="foxuries-resort foxuries-breadcrumb">
    ...
    @foreach ($resorts as $resort)
         ...
         <div class="single-room-price-wrap">
            ...
            <span class="price_value price_min" style="font-size: 36px;">{{$resort->curr}}{{$resort->price}}</span>
            <span class="unit">{{$resort->duration}}</span>
            </div>
    ...
    @foreach ($resorts as $resort)
            ...
            <div id="hb_widget_search-2" class="widget widget_hb_widget_search"><span class="gamma widget-title">Your Reservation</span>
                ...
                <ul class="hb-form-table">
                    <input type="hidden" name="resort" id="resort" value="{{$resorts[0]->id}}" />
                    @if (Auth::user())
                    ...
                    @endif
                    <input type="hidden" name="curr" id="curr" value="{{$resorts[0]->curr}}" />
                    <input type="hidden" name="price" id="price" value="{{$resorts[0]->price}}" />
    ...
    <div class="hb_room_gallery" id="hb_single_room_gallery">
        @if($resorts[0]->images)
        @foreach ($resortImages as $key => $pimg)
        
       
        <div class="inner">
            <img src="{{$pimg}}" alt=" Images {{$key + 1}}">
            </div>  
...
<p class="comment-form-email"><label for="email">Email <span class="required">*</span></label> <input id="author-email" name="email" type="email" value="" size="30" aria-required="true" /></p>
       ...
       @endif
        ...
        <input type='hidden' name='page_id' id='resort_id' value='{{$resorts[0]->id}}' />
        <input type='hidden' name='type' id='type' value='resort' />
        <p class="form-submit">
        ...
        <div id="hb_location_tab" class="hb_single_room_tab_details">
            {{$resorts[0]->address}}, {{$resorts[0]->location}}
            <div class="elementor-custom-embed">
                <iframe frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q={{$resorts[0]->location}}&amp;t=m&amp;z=10&amp;output=embed&amp;iwloc=near" aria-label="{{$resorts[0]->location}}" style="width: 100%; height: 300px;"></iframe>
            </div> 
        </div>
    ...
    <ul class="rooms tp-hotel-booking hb-catalog-column-3">
        ...
        <li id="room-{{$room->id}}" class="hb_room post-49 type-hb_room status-publish has-post-thumbnail hentry hb_room_type-lake-view">
            <div class="summary entry-summary">
            <div class="media">
            <a href="{{ url('resorts/resort/'.$room->shelter_id.'/'.$room->id.'#reservation-form') }}">
            <img src="{{$room->img_1}}" width="500" height="575" alt="" /> </a>
            </div>
            <div class="room-caption">
            <div class="title">
            <h4>
            <a href="{{ url('resorts/resort/'.$room->shelter_id.'/'.$room->id.'#reservation-form') }}">{{$room->room_no}}</a>
            </h4>
            </div>
            <div class="room-meta"><span class="room-acreage"></span></div>
            <div class="price">
            <span class="title-price">Price</span>
            <span class="price_value price_min">{{$room->curr}}{{$room->amount}}</span>
            ...
            <a href="{{ url('resorts/resort/'.$room->shelter_id.'/'.$room->id) }}" class="room-button"><i class="foxuries-icon-long-arrow-right"></i><span>View Room</span></a> </div>
            </div>
            </li> 
    ...
    <div class="foxuries-hb-room-sticky">
        ...
        <img width="75" height="65" src="{{ $resorts[0]->img_1 }}" class="attachment-foxuries-recent-post size-foxuries-recent-post wp-post-image" alt="" /> <div class="foxuries-hb-room-sticky-content">
        <span class="title">{{$resorts[0]->name}}</span>
        <span>
        <div class="price">
        <span class="title-price">Price</span>
        <span class="price_value price_min">{{$resorts[0]->curr}}{{$resorts[0]->price}}</span>
        <span class="unit">{{$resorts[0]->duration}}</span>
        </div>
        </span>
        </div>
