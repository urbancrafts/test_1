                    ...
                    'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
                      ...
                      success:function(data){
                          if(data.success == true){
                           window.location = "{{ url('service') }}/"+service+"#hb_single_room_gallery"; 
                          }else{
...
function payWithPaystack(){
    var handler = PaystackPop.setup({
      ...
      callback: function(response){
          ...
          createTransactionRecord('{{$bookings[0]->type}}','{{$bookings[0]->id}}','{{$bookings[0]->service_id}}',response.reference,'paystack','{{$bookings[0]->name}}','{{$bookings[0]->email}}','{{$bookings[0]->phone}}','{{$bookings[0]->user_id}}','{{$bookings[0]->curr}}','{{$bookings[0]->price}}','success','test transaction');
            ...
            <div class="card-payment paystack">
             ...
             <div id="paymentSection">
                <form class="adminpro-form">
                    <h4>Service: {{ $services[0]->title }}</h4>
              <h4>{{ $bookings[0]->curr }}{{ $bookings[0]->price }} / @if ($bookings[0]->duration > 1)
                {{ $bookings[0]->duration }} hours
              @else
              {{ $bookings[0]->duration }} hour 
              @endif</h4>
                    ...
                    <div class="row">
                        ...
                        <ul>
                            ...
                            <li>
                                ...
                                <input type="hidden" value="{{ $bookings[0]->service_id }}" name="service_id" id="service_id" />
                                <input type="hidden" value="{{ $bookings[0]->curr }}" name="curr" id="curr" />
                                <input type="hidden" value="{{ $bookings[0]->price }}" name="amount" id="amount" />
                                <input type="hidden" value="{{ $services[0]->title }}" name="service_name" id="service_name" />
