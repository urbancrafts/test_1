...
<script type="text/javascript">
  jQuery(document).ready(function(){
    ...
    jQuery("#shop-category").change(function(){
    if(jQuery.trim(jQuery(this).val()) == ""){
      ...
      'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
           ...
           type: "POST",
         dataType: "json",
         url: "{{ action('App\Http\Controllers\AjaxRequestController@load_boat_category') }}",
         data: {"boatCat": jQuery(this).val()},
         beforeSend:function(){
			//jQuery("#search-btn").prop('disabled', true);
			jQuery('#boat-loader').html("<div class='load'><img src='{{ asset('loaders/AjaxLoader.gif') }}' /></div>");
    ...
    function loadHomeBlog($record_per_page, $start){
      ...
      jQuery.ajax({
        ...
        'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')
           ...
           type: "POST",
           dataType: "json",
           url: "{{ action('App\Http\Controllers\AjaxRequestController@load_boat_data') }}",
  ...
  <body class="archive post-type-archive post-type-archive-hb_room wp-embed-responsive has-post-thumbnail foxuries-footer-builder elementor-default"> 
        ...
        <div class="foxuries-yacht foxuries-breadcrumb">
            ...
            <select id="shop-category" name="shop-category" style="width: 40%;">
              <option disabled selected>Types of boats</option>
    ...
    <div class="col-full">
    <ul class="rooms tp-hotel-booking hb-catalog-column-3" id="boat-loader">
