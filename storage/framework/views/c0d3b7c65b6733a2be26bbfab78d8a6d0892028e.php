...
a.book-btn, a.del-btn, a.edit-btn{
  ...
  $(document).ready(function(){
    ...
    $(function () {
      ...
      $('form#new-shop-form').on('submit', function(e){
e.preventDefault();
...
if($.trim($('#category').val()) == ""){
  ...
  $('.category-error').html('*Select Category');
     ...
     $('.shop-name-error').html("*Required"); 
}else if($.trim($('#price').val()) =="" ){
  $('#price').css('border', 'solid 1px red'); 
  $('.shop-price-error').show();
  $('.shop-price-error').html('*Required');  
}else if(!Number($('#price').val())){
     $('#price').css('border', 'solid 1px red');
     $('.shop-price-error').show()
     $('.shop-price-error').html('*Price field accepts numbers only');  
}else if($.trim($('#location').val()) == ""){
    $('#location').css('border', 'solid 1px red');
    $('.shop-location-error').show();
    $('.shop-location-error').html("*Required");
  }else{
    $.ajax({
          headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         success:function(data){
            if(data.success == true){
             ...
             $("#sendEmail").prop('disabled', false);
             $('#sendEmail').html("Submit");
             window.location = "<?php echo e(url('')); ?>/admin/shop_manager/edit_shop_img/"+data.data.id;
            //location.reload();
...
$("#shop_category_form").on("submit", function(e){
  e.preventDefault();
...
var formdata = new FormData(this);//create an instance for the form input fields
  ...
  if($.trim($('#shop_category').val()) == ""){
    $('#shop_category').css('border', 'solid 1px red');
     $('.shop-category-error').show(); 
     $('.shop-category-error').html("*Required"); 
  }else{
    $.ajax({
          headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  ...
  if(str == true){
    $.ajax({
      ...
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         type: "POST",
         dataType: "json",
         url: "<?php echo e(action('App\Http\Controllers\ServiceController@delete_service')); ?>",
         data: {"id":id},
         beforeSend:function(){  
         },
         complete:function(){  
       ...
       <?php if(Auth::user()->user_type == "admin"): ?>
          <div class="card card-info">
            <div class="card-header">
              ...
              <h3 class="card-title"><i class="ion ion-bag"></i> Create Shop Categories</h3>
              <!-- tools box -->
            ...
            <div class="card-body">
              <form id="shop_category_form" action="<?php echo e(action('App\Http\Controllers\ShopController@create_categories')); ?>" method="post" enctype="multipart/form-data">
                
                
                <div class="form-group">
                    <span style="color: red; display: none" class="shop-category-error">(*Required)</span>
                  <input type="text" class="form-control" id="shop_category" name="shop_category" onfocus="elementFocus(this.id, 'shop-category-error')" placeholder="Enter Shop Category">
                </div>
          ...
          <?php endif; ?>
            ...
            <div class="card card-info">
                <div class="card-header">
                  ...
                  <h3 class="card-title"><i class="ion ion-bag"></i> New Shop Product</h3>
                  <!-- tools box -->
                  <div class="float-right card-tools">
                ...
                <div class="card-body">
                  <form id="new-shop-form" action="<?php echo e(action('App\Http\Controllers\ShopController@create_new_product')); ?>" method="post" enctype="multipart/form-data">
                   ...
                   <div class="form-group">
                    <label>Item Name:</label>
                        <span style="color: red;" class="shop-name-error">*</span>
                      <input type="text" class="form-control" id="name" name="name" onfocus="elementFocus(this.id, 'shop-name-error')" placeholder="Enter your item name">
                    </div>
                    ...
                    <div class="form-group">
                      <label>Price:</label>
                        <span style="color: red;" class="shop-price-error">*</span>
                        <input type="number" class="form-control" id="price" name="price" onfocus="elementFocus(this.id, 'shop-price-erro')" placeholder="price">
                      </div>
                      </div>
                    </div>
                    ...
                    <div class="row">
                      ...
                      <div class="form-group">
                        ...
                        <span style="color: red;" class="shop-location-error">*</span>
                        <input type="text" class="form-control pac-target-input valid" id="location" name="location" onfocus="elementFocus(this.id, 'shop-location-error')" data-validation="required" placeholder="Enter a location" autocomplete="off">
                      ...
                      <div class="form-group">
                        ...
                        <span style="color: red; display: none" class="shop-youtube-error">(*Required)</span>
                    ...
                    <div class="form-group">
                      <label>Description:</label>
                        <span style="color: red; display: none" class="shop-desc-error">(*Required)</span>
            ...
            <div class="card card-primary card-outline">
                ...
                <div class="card-body">
                  <table id="example1" class="products-list product-list-in-box">
                    ...
                    <tbody>
                 <?php $__currentLoopData = $stores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shop): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                 <?php if(Auth::user()->role == 1 || Auth::user()->user_type == "admin" || $shop->created_by == Auth::user()->id): ?>
                     
                 <tr class="item" >
                  <td width='500'>
                  <div class="product-img">
                    <img src="<?php echo e($shop->img_1); ?>" >
                  </div>
                  <div class="product-info">
                    <a href="<?php echo e(url('shop/product/'.$shop->id)); ?>" class="product-title"><?php echo e($shop->item_name); ?>

                      
                      
                      <span class="badge badge-warning float-right">
                          <?php echo e($shop->curr); ?><?php echo e($shop->price); ?>

                        </span>
                       
                      </a>
                    <span class="product-description">
                          <?php echo e($shop->category); ?>

                        </span>
                       
                        <a href="#" class="fa fa-trash del-btn" onclick="deleteService('<?php echo e($shop->id); ?>','<?php echo e($shop->item_name); ?>')" title="Delete <?php echo e($shop->item_name); ?>">Delete</a> | 
                        <a href="<?php echo e(url('admin/shop_manager/edit_shop/'.$shop->id)); ?>" class="fa fa-edit edit-btn" title="Edit <?php echo e($shop->item_name); ?>">Edit</a> | 
                        <a href="<?php echo e(url('admin/shop_manager/product_order/'.$shop->id)); ?>" class="fa fa-book book-btn" title="Check <?php echo e($shop->item_name); ?> orders">Orders</a>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\beach_comber\resources\views/home/create_shop.blade.php ENDPATH**/ ?>