...
$(document).ready(function(){
  ...
  $('form#discount-form').on('submit', function(e){
e.preventDefault();
...
if($.trim($('#discount-number').val()) == ""){
     $('#discount-number').css('border', 'solid 1px red');
     $('.shop-discount-number-error').show();
     $('.shop-discount-number-error').html('*Required');
}else if($.trim($('#discount-percent').val()) == ""){
     $('#discount-percent').css('border', 'solid 1px red');
     $('.shop-discount-percent-error').show();
     $('.shop-discount-percent-error').html('*Required');  
     ...
     $.ajax({
      ...
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
         ...
         beforeSend:function(){
             $("#save-btn").prop('disabled', true);
             $('#save-btn').html("<img src='<?php echo e(asset('loaders/AjaxLoader.gif')); ?>' />");
         ...
         error:function(){
          ...
          $("#save-btn").prop('disabled', false);
          $('#save-btn').html("SAVE AND CONTINUE");
         },
         success:function(data){
            if(data.success == true){
             ...
             $("#save-btn").prop('disabled', false);
             $('#save-btn').html("SAVE AND CONTINUE");
             window.location = "<?php echo e(url('')); ?>/admin/shop_manager/edit_delivery_settings/"+shop;
...
<div class="wrapper">   
          ...
          <div class="tabSide">
            <div class="card card-body p-0" style="border: 2px solid #E51924; border-radius: 0px">
                <ul class="nav nav-pills nav-justified">
                  ...
                  <li class="nav-item">
                    <a class="nav-link active" href="<?php echo e(url('admin/shop_manager/edit_discount_settings/'.$shops[0]->id)); ?>">Discount</a>
                  </li>
                  <li class="nav-item">
  ...
  <!-- <li class="nav-item">
   ...
   -->
          ...
          <form id="discount-form" method="post" action="<?php echo e(action('App\Http\Controllers\ShopController@update_product_discount')); ?>">
            <input hidden name="shop" id="shop" value="<?php echo e($shops[0]->id); ?>" />
            <div class="form-group">
              <fieldset class="resort-features">
                <legend>Discount Settings</legend>
                
                <label><input type="checkbox" name="settings" id="settings" />Set Discount</label>
                  ...
                  <div class="row">
                    <div class="col-sm-6">
                      ...
                      <div class="form-group">
                        ...
                        <span style="color: red;" class="shop-discount-number-error">*</span>
                          <input type="number" id="discount-number" name="discount-number" class="form-control" onfocus="elementFocus(this.id, 'shop-discount-number-error')" placeholder="Enter discount number: 10" value="<?php echo e($shops[0]->discount_number); ?>" >
                      </div>
                    </div>
                    <div class="col-sm-6">
                  <div class="form-group">
                    <label>Discount Percent:</label>
                      <span style="color: red;" class="shop-discount-percent-error">*</span>
                      <input type="number" class="form-control" id="discount-percent" name="discount-percent" onfocus="elementFocus(this.id, 'shop-discount-percent-error')"  placeholder="Enter percent: 10(%)" value="<?php echo e($shops[0]->discount_percent); ?>" >
                    </div>
                    </div>
                  </div>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\beach_comber\resources\views/home/edit_discount_settings.blade.php ENDPATH**/ ?>